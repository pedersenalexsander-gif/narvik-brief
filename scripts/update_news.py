...
OSLO=ZoneInfo("Europe/Oslo"); MAX_STORIES=10; MAX_AGE_HOURS=72; MIN_ARTICLE_CHARS=1000
...
def fetch_bytes(url,timeout=25,accept="*/*"):
    req=urllib.request.Request(url,headers={"User-Agent":"Mozilla/5.0 (compatible; AlexBrief/9.0)","Accept-Language":"nb-NO,nb;q=0.9,en;q=0.8","Accept":accept})
...
def discover():
    ...
    for category,url in list(RSS_FEEDS)+[(c,google_news_url(q)) for c,q in GOOGLE_QUERIES]:
        ...
        for item in feed:
            ...
            item["score"]=max(0,int(14-age/5))+sum(w for k,w in IMPORTANT.items() if k in f"{title} {item.get('description','')}".lower())+(3 if category=="AI" else 0); item["id"]=story_id(title,item["source"]); items.append(item)
    return items
def dedupe(items):
    seen=[];out=[]
...
def looks_english(t):
    low=f" {t.lower()} "; hits=sum(low.count(f" {w} ") for w in ("the","and","with","from","said","has","have","will","after","for","this","that")); return hits>=2
def translate_no(t):
...
def localize_story(story):
    story=dict(story); original=story.get("image",""); local=story.get("localImage","")
    if local and (ROOT/local).exists():return story
    if original: story["localImage"]=localize_image(original,story.get("id","story"))
    return story
def translate_story(story):
    story=dict(story)
    if looks_english(story.get("title","")+" "+story.get("summary","")):
        story["title"]=translate_no(story.get("title","")); story["summary"]=translate_no(story.get("summary","")); story["keyPoints"]=[translate_no(x) for x in story.get("keyPoints",[])]; story["translatedToNorwegian"]=True
    return story
def load_previous():
    try:return json.loads(OUT.read_text(encoding="utf-8"))
    except Exception:return {"stories":[]}
def build_story(item,prev):
    old=prev.get(item["id"])
    if old and old.get("summary") and old.get("summaryMethod")=="local-extractive": return translate_story(localize_story(old))
    ...
    summary,points,why=generated; published=item["published_dt"].astimezone(OSLO); image=page["image"] or item.get("feed_image","")
    story={"id":item["id"],"category":item["category"],"published":published.strftime("%d.%m. %H:%M"),"publishedISO":published.isoformat(),"title":title,"summary":summary,"keyPoints":points,"whyItMatters":why,"summaryMethod":"local-extractive","url":page["url"],"source":item["source"],"image":image,"localImage":localize_image(image,item["id"])}
    return translate_story(story)
def select_balanced(stories):
    chosen=[];ids=set()
    for c,q in CATEGORY_QUOTAS.items():
        pool=sorted([s for s in stories if s.get("category")==c and s.get("id") not in ids],key=lambda s:s.get("publishedISO",""),reverse=True)
        for s in pool[:q]:chosen.append(s);ids.add(s.get("id"))
    rest=sorted([s for s in stories if s.get("id") not in ids],key=lambda s:s.get("publishedISO",""),reverse=True);chosen.extend(rest[:MAX_STORIES-len(chosen)]);chosen.sort(key=lambda s:s.get("publishedISO",""),reverse=True);return chosen[:MAX_STORIES]
def main():
    previous=load_previous(); prev={s.get("id"):s for s in previous.get("stories",[]) if s.get("id")}; built=[]
    for item in dedupe(discover()):
        s=build_story(item,prev)
        if s:built.append(s)
        if len(built)>=24:break
    cutoff=datetime.now(timezone.utc)-timedelta(hours=MAX_AGE_HOURS)
    ids={s.get("id") for s in built}
    for old in previous.get("stories",[]):
        if old.get("id") in ids:continue
        try:p=datetime.fromisoformat(old.get("publishedISO","")).astimezone(timezone.utc)
        except Exception:continue
        if p>=cutoff and old.get("summaryMethod")=="local-extractive":built.append(translate_story(localize_story(old)))
    selected=select_balanced(built)
    if not selected:print("Ingen nye kvalitetssaker. Beholder forrige brief.");return
    now=datetime.now(OSLO); payload={"updatedAt":now.strftime("%d.%m.%Y kl. %H:%M"),"updatedISO":now.isoformat(),"qualityMode":"multi-rss-full-article-free-local-images-no","stories":selected}; OUT.write_text(json.dumps(payload,ensure_ascii=False,indent=2)+"\n",encoding="utf-8"); print(f"Publiserte {len(selected)} saker, med lokale bilder og norsk visning der oversettelse var nødvendig.")
